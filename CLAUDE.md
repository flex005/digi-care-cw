...
- **Every card has an expand button** in its top right corner: a 36px grey circle with a diagonal arrow. `CardHead` requires it, and a card with nowhere to go says so when it is tried rather than dropping the button.
...
- **A hook that works because of something unrelated to it is a bug waiting for that thing to change.** The residents list's filter memo omitted the search text. It worked here only because the list it filtered was rebuilt on every render, which recomputed the memo anyway; the Admin build, where the list keeps its identity, has never narrowed on search. Tests passed in both, because nothing typed into the search. What caught it was a lint warning read as a warning rather than noise. **When a dependency list is incomplete, write the test that fails with the list as it stands under the conditions that make it live**, here a stable list, and only then add the dependency.
