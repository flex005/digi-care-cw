...
- **A note in DEPARTURES is a claim about the PRD, and it gets checked against the PRD.** Phase 9 recorded that Risk assessments, Consent and Documents had no home-wide screen because "a home-wide screen for each is not in the CW PRD". All three are in it — RA-01, CON-01 and DOC-01, full screen specifications of the same shape as three that had been built without hesitation. The sentence was written from memory of what the build contained rather than from the document, and once written it read as a decision, so the next phase inherited a reason where there had only been an omission. Three rail entries had pointed at nothing since Phase 0 and nothing broke, because `isBuilt()` draws "not built yet" rather than a dead link: the build was honest to a reader and silent to its authors. **Quote the document in the departure, or go and read it before writing the line** — and where the reason for not building something is "the PRD does not ask", that is the claim most worth checking, because it is the one that stops anybody looking again. `check-nav-reach.mjs` now makes the silent half loud: a way into a module with no screen fails the build unless somebody names it and says why.

- **A date written into a test is true for one day, and the suite says so the next morning.** The incident form's tests held `'2026-09-18T20:20'` as the expected default and typed `'2026-09-19T08:00'` as a time "in the future". Both passed for as long as the calendar agreed with them; the day the real date rolled to the 19th, the first expected yesterday and the second's future time became the past, and two tests failed on a tree nobody had touched. **The fixture clock is built against the instant the page loads**, so anything a screen derives from it moves daily while a literal does not — and the tell is a test that reads `now()` and asserts against something typed by hand. Derive the expectation from the same clock the screen reads, and where a test needs a time in the future, measure it from that clock rather than naming a date. A self-contained test that supplies its own `now` alongside its own records is not this: there, the literals and the clock move together because both are the test's.

- **`composes:` does not chain, and the element it fails on renders to almost nothing.** The MAR legend drew two 2px squares where 34px ones belonged: `.cellQuiet { composes: cellNotDue }` where `.cellNotDue { composes: cell }` puts `cellQuiet cellNotDue` on the element and **not** `cell`, so the grandparent's width, height and `display: flex` were simply absent and a 1px border round a zero box was all that was left. Nothing said so — both classes exist, `check-css-classes` was happy, the typechecker has no opinion, and the screen rendered. It was found by a person looking at a legend and saying two entries had no swatch, and named by measuring the elements rather than by reading the CSS, which looked correct. **The tell is a composition whose target is itself a composition**; `check-css-classes.mjs` now fails on one and says which base to name directly. The wider shape: a guard that checks a name exists is not checking that the name carries what its author thought it carried.

- **A probe that fires the wrong event reports a working component as broken, and the next move is to fix something that was never wrong.** The top bar's new bell and app switcher would not open in jsdom, so the failure was chased into the `Tooltip` primitive — which does drop the props an `asChild` parent hands it — and that was changed. The menus still did not open. A check in a real browser, driven with `element.click()`, agreed they were shut; the same check driven with the pointer sequence Radix actually listens for (`pointerdown` first) opened both, and had done all along. The speculative fix was reverted. **Two tells, either of which was enough**: the trigger already carried `aria-haspopup="menu"`, so the composition was reaching the element; and the thing under test opens on `pointerdown` while the probe sent `click`. Before changing a component because a test says it is broken, make the probe fire what the component listens for, and confirm the probe can pass at all — here, against the account menu beside it, which has no tooltip and works.
